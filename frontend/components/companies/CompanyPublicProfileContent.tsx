...
type Props = {
  /** Link back to the browse companies list for this role. */
  listHref: string;
};

export function CompanyPublicProfileContent({ listHref }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data: row } = await supabase
        .from("companies")
        .select("id, company_name, description, location, industry, logo_url, user_id, is_new_company")
        .eq("id", id)
        .maybeSingle();
  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-[#F8FAFC] py-8 transition-colors duration-300 dark:bg-slate-950">
    ...
    return (
      <main className="min-h-screen bg-[#F8FAFC] px-4 py-16 transition-colors duration-300 dark:bg-slate-950">
        <div className="mx-auto max-w-lg rounded-2xl border border-[#E2E8F0] bg-white p-8 text-center dark:border-slate-800 dark:bg-slate-900">
          ...
          <Link href={listHref} className="mt-4 inline-block text-sm font-medium text-[#7C3AED] hover:text-[#6D28D9] dark:text-violet-400">
            ← Back to companies
          </Link>
        </div>
      </main>
  ...
  const industryLoc = [company.industry ? formatIndustryLabel(company.industry) : null, company.location]
    .filter(Boolean)
    .join(" · ");
  const isNewCompany = company.is_new_company == null ? true : Boolean(company.is_new_company);
    ...
    <main className="min-h-screen bg-[#F8FAFC] transition-colors duration-300 dark:bg-slate-950">
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        <div className="-mt-16 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex items-end gap-4">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                {isNewCompany ? <Badge variant="warning">New</Badge> : null}
              </div>
          ...
          <div className="flex flex-wrap gap-2">
            <MessageCompanyButton
              companyOwnerUserId={company.user_id ?? ""}
        ...
        <section className="mt-10 rounded-2xl border border-[#E2E8F0] bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <h2 className="text-lg font-semibold text-[#0F172A] dark:text-white">Company evaluation</h2>
          <div className="mt-4">
            {canViewEvaluation ? (
        ...
        <section className="mt-8 rounded-2xl border border-[#E2E8F0] bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <h2 className="text-lg font-semibold text-[#0F172A] dark:text-white">Student feedback</h2>
          <p className="mt-1 text-sm text-[#0F172A]/70 dark:text-slate-400">
            Ratings and notes from students who completed internships here.
          </p>
          <div className="mt-4">
            {canViewEvaluation ? (
              <CompanyStudentFeedbacks companyId={company.id} />
            ) : (
              <p className="text-sm text-[#0F172A]/70 dark:text-slate-400">
                Sign in to read student reviews for this company.
              </p>
            )}
          </div>
        </section>
        ...
        <section className="mt-8 rounded-2xl border border-[#E2E8F0] bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="mt-3 text-sm text-[#0F172A]/80 dark:text-slate-300">
            {company.description?.trim() ? company.description : "This company has not added a description yet."}
          </p>
        </section>
        ...
        <p className="mt-8 pb-10">
          <Link href={listHref} className="text-sm font-medium text-[#7C3AED] hover:text-[#6D28D9] dark:text-violet-400">
            ← Back to companies
          </Link>
        </p>
      </div>
