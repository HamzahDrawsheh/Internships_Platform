...
export function NextActionCard({ action }: Props) {
  ...
  return (
    <div className={`rounded-2xl border p-5 ${urgencyStyles[action.urgency]}`}>
      <p className="text-xs font-semibold uppercase tracking-wide text-purple-700 dark:text-purple-300">Next action</p>
      <h3 className="mt-1 text-lg font-semibold text-gray-900 dark:text-white">{action.title}</h3>
      ...
      <Link href={action.href} className="mt-4 inline-block">
        <Button variant="primary">{action.ctaLabel}</Button>
      </Link>
    </div>
  );
}
